...
import { ParticipantsContext, BuildingContext, DateTimeContext } from "../context";
...
function Heatmap() {
    ...
    function drawParticipantsLocation() {
        ...
        const imageScaleX = d3.scaleLinear()
            .domain([0, imageWidth])
            .range([pointsRadius * 2 + pointsRadius, svgDimention.width - pointsRadius]);
        const imageScaleY = d3.scaleLinear()
            .domain([0, imageHeight])
        ...
        const participants = svg.selectAll("circle.participant").data(participantsLocation);
